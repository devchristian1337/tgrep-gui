...
The script publishes x64 and ARM64 as self-contained single-file executables with `tgrep.exe` bundled inside, then zips each host as `artifacts\tgrep-gui-win-x64.zip` and `artifacts\tgrep-gui-win-arm64.zip`. The zip contains only `tgrep-gui.exe`; keep that file name. Trimming is not used, so XAML and bindings stay reliable. The first launch extracts dependencies to a temporary folder.
